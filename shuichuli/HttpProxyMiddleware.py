...
class HttpProxyMiddleware(object):
    ...
    def __init__(self, use_https, agents_list):
        # 保存上次不用代理直接连接的时间点
        ...
        self.proxy_file = "HttpProxyMiddlewareTest/proxyes.dat"
        # 是否在超时的情况下禁用代理
        ...
        # 初始化代理列表
        self.proxyes = [{"proxy": None, "valid": True, "count": 0 ,"retry_count": 0}]
        # 初始时使用0号代理(即无代理)
        self.proxy_index = 0
        ...
        if os.path.exists(self.proxy_file):
            with open(self.proxy_file, "r",encoding="utf8") as fd:
                lines = fd.readlines()
                for line in lines:
                    line = line.strip()
                    ...
                    self.proxyes.append({"proxy": line,
                                        ...
                                        "count": 0,
                                         "retry_count": 0})
        print("-----------------代理个数：",len(self.proxyes))

    @classmethod
    def from_crawler(cls, crawler):
        use_https = crawler.settings.getbool('HTTPS_PROXY')
        agents_list = crawler.settings.getlist('USER_AGENTS')
        return cls(use_https,agents_list)
    ...
    def fetch_new_proxyes(self):
        ...
        从网上抓取新的代理添加到代理列表中
        """
        print("有效ip个数：", self.len_valid_proxy(), "最小限制个数：", self.extend_proxy_threshold, "开始获取新代理==============")
        logger.info("extending proxyes using fetch_free_proxyes.py")
        new_proxyes = fetch_free_proxyes.fetch_all(endpage=5,https=self.use_https)
        print(new_proxyes)
        ...
        print("==================获取完毕，现有效代理个数：", self.len_valid_proxy())
    ...
    def inc_proxy_index(self, current=-1):
        ...
        if self.proxy_index == 0 and datetime.now() < self.last_no_proxy_time + timedelta(minutes=5):
    ...
    def set_proxy(self, request):
        ...
        """
        import random
        if "proxy" in request.meta and request.meta["proxy_index"] != self.proxy_index:
            print("-----切换代理------")
        ...
        if self.proxy_index == 0: # 每次不用代理直接下载时更新self.last_no_proxy_time
            self.last_no_proxy_time = datetime.now()
            print("没有使用代理","url:",request.url)

        if proxy["proxy"]:
            request.meta["proxy"] = proxy["proxy"]
            print("代理ip:",proxy["proxy"],"url:",request.url)
    ...
    def process_request(self, request, spider):
        ...
        if "change_proxy" in request.meta and request.meta["change_proxy"]:
            print("处理获取失败的url",request.url)
            logger.info("change proxy request get by spider: %s" % request)
            if self.proxyes[request.meta["proxy_index"]]:
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200 \
            ...
            self.proxyes[request.meta["proxy_index"]]["retry_count"] = 0
            self.proxyes[request.meta["proxy_index"]]["count"] += 1
            print("--------------",self.proxyes[request.meta["proxy_index"]]["proxy"],self.proxyes[request.meta["proxy_index"]]["count"])
            ...
            print("======response cookies====",cookie_dict)
            print("======request cookies====",request.headers.getlist('Cookie'))
            return response
    ...
    def process_exception(self, request, exception, spider):
        ...
        处理由于使用代理导致的连接异常
        """
        # print("代理连接错误！",request.meta['proxy'],request.url,self.proxyes[request.meta['proxy_index']]['count'])
        ...
        # print(self.proxyes[request.meta["proxy_index"]]["retry_count"])
        print("*******request cookie******",request.cookies)
        if self.proxyes[request.meta["proxy_index"]]["retry_count"] >= 10:
            self.invalid_proxy(request.meta["proxy_index"])
        else:
            self.proxyes[request.meta["proxy_index"]]["retry_count"] += 1

        new_request = request.copy()
        new_request.dont_filter = True
        new_request.meta["rec_proxy"] = True
        return new_request


class Cookiesself(CookiesMiddleware):
    def process_response(self, request, response, spider):
